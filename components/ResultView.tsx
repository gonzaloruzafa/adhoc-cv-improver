                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      const file = new File([imageBlob], 'mi-cv-ranking-adhoc.png', { type: 'image/png' });
                      
                      // Always download the image
                      const url = URL.createObjectURL(imageBlob);
                      const a = document.createElement('a');
                      a.href = url;
                      a.download = 'mi-cv-ranking-adhoc.png';
                      a.click();
                      URL.revokeObjectURL(url);
                      
                      // Copy text with URL for easy posting
                      const shareText = `¡Logré ${result.ranking.score} puntos en mi CV! 🎯 Nivel: ${result.ranking.nivel}

Usé el analizador de CVs de Adhoc y me ayudó a mejorar mi perfil profesional.

¿Querés optimizar tu CV también? Probalo acá: ${window.location.origin}`;
                      
                      await navigator.clipboard.writeText(shareText);
                      alert('✅ Imagen descargada y texto copiado!\n\nAhora podés:\n1. Crear un post en LinkedIn\n2. Adjuntar la imagen descargada\n3. Pegar el texto copiado');
                    } catch (err) {
                      console.error('Error sharing:', err);
                      alert('Hubo un error. Intentá de nuevo.');
                    }
                  }}
                  className="w-full py-3 bg-white/20 hover:bg-white/30 text-white rounded-lg font-secondary font-medium transition-all border border-white/30 flex items-center justify-center gap-2"
                >
